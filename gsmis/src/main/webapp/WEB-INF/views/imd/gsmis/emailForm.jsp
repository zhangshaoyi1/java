			...
			$("#btnSubmit").click(function(){
				...
				if(str.indexOf("{1}")<0 || str.indexOf("{2}")<0 || str.indexOf("{3}")<0 || str.indexOf("{4}")<0){
					top.$.jBox.confirm("{1}、{2}、{3}、{4}是关键字，请不要删除这些关键字","系统提示",function(v,h,f){
						if(v=="ok"){
							$("#searchForm").attr("action","${ctx}/gsmis/email/form");
							$("#searchForm").submit();
						}
					},{buttonsFocus:1});
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/gsmis/email/">邮件模板列表</a></li>
		<li class="active"><a href="${ctx}/gsmis/email/form?id=${email.id}">邮件模板<shiro:hasPermission name="gsmis:email:edit">${not empty email.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="gsmis:email:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="email" action="${ctx}/gsmis/email/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<sys:ckeditor replace="text" uploadPath="/gsmis/email" />
		...
		<div class="form-actions">
			<shiro:hasPermission name="gsmis:email:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
