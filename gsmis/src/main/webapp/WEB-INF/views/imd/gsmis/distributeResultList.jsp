		...
		$(document).ready(function() {
			 $("#btnsend").click(function(){
				top.$.jBox.confirm("确定要一键给所有专家发送邮件吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/gsmis/email/send");
						$("#searchForm").submit();
					}
				},{buttonsFocus:1});
			 ...
			 $("#btnselectsend").click(function(){
					top.$.jBox.confirm("确定要给所选中的专家发送邮件吗？","系统提示",function(v,h,f){
						if(v=="ok"){
							...
							$("#searchForm").attr("action","${ctx}/gsmis/email/sendSelectEmail?distributeResultIds="+distributeResultIds);
							$("#searchForm").submit();
						}
					},{buttonsFocus:1});
					top.$('.jbox-body .jbox-icon').css('top','55px');
...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/gsmis/distributeResult/">分配结果列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="distributeResult" action="${ctx}/gsmis/distributeResult/list" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<shiro:hasPermission name="gsmis:distributeResult:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="distributeResult">
			<tr>
				...
				<shiro:hasPermission name="gsmis:distributeResult:edit">
					<td>
					<a href="${ctx}/gsmis/distributeResult/delete?id=${distributeResult.id}" onclick="return confirmx('确认要删除该分配结果吗？', this.href)">删除</a>
					</td>
				</shiro:hasPermission>
			</tr>
		</c:forEach>
