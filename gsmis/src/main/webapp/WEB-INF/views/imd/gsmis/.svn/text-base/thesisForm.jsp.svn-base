			...
			if(idx<=1){
				...
				return function(){
					top.$.jBox.confirm("一条论文记录至多只能添加两条用户数据！","系统提示",function(v,h,f){
						if(v=="ok"){
							$("#searchForm").attr("action","${ctx}/gsmis/thesis/form");
							$("#searchForm").submit();
						}
					},{buttonsFocus:1});
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/gsmis/thesis/">论文信息列表</a></li>
		<li class="active"><a href="${ctx}/gsmis/thesis/form?id=${thesis.id}">论文信息<shiro:hasPermission name="gsmis:thesis:edit">${not empty thesis.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="gsmis:thesis:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="thesis" action="${ctx}/gsmis/thesis/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
				...
				<div class="controls">
					<table id="contentTable" class="table table-striped table-bordered table-condensed">
						<thead>
							<tr>
								...
								<th>密码</th>
								<th>备注信息</th>
								<shiro:hasPermission name="gsmis:thesis:edit"><th width="10">&nbsp;</th></shiro:hasPermission>
							</tr>
						</thead>
						<tbody id="accountList">
						</tbody>
						<shiro:hasPermission name="gsmis:thesis:edit"><tfoot>
					...
					<script type="text/template" id="accountTpl">//<!--
						<tr id="accountList{{idx}}">
							...
							<shiro:hasPermission name="gsmis:thesis:edit"><td class="text-center" width="10">
		...
		<div class="form-actions">
			<shiro:hasPermission name="gsmis:thesis:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
